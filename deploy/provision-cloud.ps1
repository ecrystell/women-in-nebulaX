...
param(
  ...
  [string]$RuntimeAccountId = "railaccess-runtime"
)
...
if (-not $existing) {
  gcloud iam service-accounts create $RuntimeAccountId --project $ProjectId --display-name "RailAccess Cloud Run runtime"
}
...
$repository = gcloud artifacts repositories list --project $ProjectId --location us-central1 --filter "name:railaccess" --format "value(name)"
if (-not $repository) {
  gcloud artifacts repositories create railaccess --project $ProjectId --location us-central1 --repository-format docker --description "RailAccess hackathon images"
}
