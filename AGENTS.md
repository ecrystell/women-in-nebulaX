...
| A/B/C API run gate | Complete | Solver input/topology failures return `failed/solver_input_invalid`; unexpected solver errors return `failed/solver_failed`. A candidate is locally preflighted before success. Hard findings retain schedule/report evidence but return `failed/preflight_failed`, create no exports, and download attempts return `409/preflight_not_clean`. API evidence includes CP-SAT status, elapsed time, and the 150-second limit. |
...
- The hosted adapter accepts all delivered A/B/C policies using the shared `PreparedInstance`, with a 150-second maximum solve budget and no scenario fallback.
