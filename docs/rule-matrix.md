...
| R04 | Live/Non-live(Consist) work applies the published buffer; Non-live(Others) has none. Buffers and external work may not overlap. | `05_BUFFER_LOCATION`, R03 route footprint, project nature, occupancies | Extend Consist through buffer tunnel sectors; extend Live through buffer tunnels and platforms; reject same-week base-vs-closure collisions unless legal co-share applies. | `closure` | Consist work exposes its one-sector tunnel buffer; Live exposes buffered platforms and tunnels. | Role 2 | Implemented locally |
| R05 | Live work mirrors onto the opposite bound. | `05.opposite_bound_required`, project nature, R03 footprint | Add opposite-bound closure footprint and attach it as local evidence. | `closure` | Live EB work exposes WB in its footprint. | Role 2 | Footprint evidence fixture complete; collision timing unresolved |
| R06 | A Live activity at H01–H02 also closes the other line's H01–H02 tunnel and H01/H02 platforms; non-Live work does not cross lines. | `02.is_interchange`, `03_SECTORS`, project nature, R03 footprint | Add cross-line H01/H02 Live closure and attach it as local evidence. | `closure` | Live ALP H01–H02 work exposes BET H01–H02 footprint; non-Live stays line-local. | Role 2 | Footprint evidence fixture complete; collision timing unresolved |
...
| R08 | Same location/week/co-share group is one possession and is exempt from each other's buffers; an activity may not sit inside another submitted possession group's closure zone in the same week. | occupancy rows; R04 buffer footprint; `04_LOCATION_SUPPLY` | Reject location-specific directional base-to-closure overlap for Scenario C unless the activities form one legal C/C or PC/C co-share possession at that location; retain hidden weekly physical-night colouring for the remaining capacity/order check. `access_night` remains contract/type-local and is not reused as a global identifier. | `closure`; organiser message `[Activity inside another group's closure zone]` | The organiser-rejected Scenario C output produced 62 such errors; the current corrected Scenario C incumbent produces zero local closure findings. | Role 2 | Implemented from recorded organiser evidence; A retains directional findings as diagnostics |
...
| R14 | Scenario B: supply excess is allowed but counted; ECLO is allowed and counted. | R11 capacity count, placement `eclo` | Compute excess access-nights and ECLO nights for score; no hard cap on excess. | `capacity` is soft-scored; exact report tag confirmed by validator | B schedule uses excess and ECLO; assert `7 × excess + 5 × ECLO` metrics, not a hard failure. | Role 2 | Local metrics fixture complete; organiser score semantics pending |
| R15 | Scenario C: no more than one excess access-night per location/week. | R11 capacity count, scenario | Require possessions ≤ nominal supply + 1; count allowed excess in score. | `capacity` | Supply 0 accepts one possession but rejects two separate groups in C. | Role 2 | Local `+1` boundary fixture complete |
| R16 | Scenario C: ECLO for each line fits one continuous window of at most two calendar weeks; cross-line Live ECLO fits both affected-line windows. | placement `eclo`, R03/R05/R06 line footprint, scenario | Determine affected line(s), collect ECLO weeks, require each line's span ≤ 2 weeks and include shared footprint evidence. | `eclo` or continuity tag TBD | Same-line ECLO in weeks 1 and 4; cross-line Live ECLO yields both line findings. | Role 2 | Local-evidence fixtures complete |
| R17 | Scenario A objective is priority-weighted planned-completion overrun; B objective is `7 × excess + 5 × ECLO`; C combines both. | `07.contract_priority`, `08.activity_priority`, planned dates, completion, R11/R14 counts, eclo | Calculate published score terms exactly; hard constraints are never hidden by score. | Score field names/tag TBD | Public-fixture score components and published weights are asserted locally. | Role 3 | Local diagnostics/fixtures complete; confirm score semantics with organiser |
